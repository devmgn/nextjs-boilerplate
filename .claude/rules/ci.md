...
- `main` + feature branches. Workflows trigger on `push` with `branches-ignore: main`, except **actions-lint.yml** (every branch) and **apm-lock.yml** (`renovate/**` only)
- **lint.yml**: `pnpm check` — **test.yml**: `pnpm test:unit` + `pnpm test:oxlint-rules` — **build.yml**: `pnpm build` + upload `.next/`
- Path-scoped: **actions-lint.yml** (`.github/**`), **adr-check.yml** (`docs/adr/**`), **apm-check.yml** / **apm-lock.yml** (`apm.yml`, `apm.lock.yaml`)
